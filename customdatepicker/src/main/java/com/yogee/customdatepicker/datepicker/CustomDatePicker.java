package com.yogee.customdatepicker.datepicker;
...
import com.yogee.customdatepicker.R;
...
public class CustomDatePicker implements View.OnClickListener, PickerView.OnSelectListener {
    ...
    private TextView mTvYearUnit,mTvMontyUnit,mTvDayUnit,mTvHourUnit, mTvMinuteUnit;
    ...
    private void initView() {
        ...
        mTvMontyUnit = mPickerDialog.findViewById(R.id.tv_month_unit);
     ...
     * 不显示时分的情况下去掉日的显示
     */
    public void setCanShowDay(boolean canShowPreciseTime) {
        if (!canShow()) return;
     ...
     * 设置日期控件是否显示日期
     */
    public void setMissPreciseDate(boolean missPreciseDate) {
        if (!canShow()) return;
        ...
        if (missPreciseDate) {
            ...
            mTvMontyUnit.setVisibility(View.GONE);
            mDpvDay.setVisibility(View.GONE);
            mTvDayUnit.setVisibility(View.GONE);
            mDpvHour.setVisibility(View.VISIBLE);
            mTvHourUnit.setVisibility(View.VISIBLE);
