...
public class WhpbShaixActivity extends BaseActivity {
    ...
    private DialogBotList mDialogBotList_lingqu;
    private DialogBotList mDialogBotList_kaitong;
    ...
    private List<BotListBean> mBotListBeans_lingqu;
    private List<BotListBean> mBotListBeans_kaitong;
    private String city_code, quyu_code, shangquan_code, one_code, two_code, lq_code, kait_code, shangquan_type;
    private CustomDatePicker customDatePickerSt;
    private String[][] mRepeat = new String[][]{{"张三", "1"}, {"李四", "2"}, {"王五", "2"}, {"张三三", "2"}, {"李四四", "2"}};
    private String[][] mLingq = new String[][]{{"领取", "1"}, {"未领取", "2"}};
    private String[][] mkait = new String[][]{{"开通", "1"}, {"未开通", "2"}};
    ...
    public void init() {
        ...
        mBotListBeans_lingqu = new ArrayList<>();
        mBotListBeans_kaitong = new ArrayList<>();
    ...
    @OnClick({R.id.imgv_back, R.id.screen_reset, R.id.screen_shaix, R.id.screen_chongm_tuozhan, R.id.screen_chongm_quany, R.id.screen_chongm_jingl,
            R.id.screen_lingqstate, R.id.screen_kaitstate, R.id.screen_yiji, R.id.screen_erji, R.id.screen_city, R.id.screen_quyu, R.id.screen_shangquan,
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            ...
            case R.id.screen_lingqstate:
                if (mDialogBotList_lingqu != null)
                    mDialogBotList_lingqu.show();
                break;
            case R.id.screen_kaitstate:
                if (mDialogBotList_kaitong != null)
                    mDialogBotList_kaitong.show();
                break;
        }
    }
    ...
    private void toReset() {
        ...
        mStartTm.setText("");
        mEndTm.setText("");
        mScreenLingqstate.setText("");
        mScreenKaitstate.setText("");
    }
    ...
    private void toJump() {
        ...
        String liq = mScreenLingqstate.getText().toString();
        String kait = mScreenKaitstate.getText().toString();
        if (TextUtils.isEmpty(name) && TextUtils.isEmpty(tuozh) && TextUtils.isEmpty(quany) && TextUtils.isEmpty(xiaoer) && TextUtils.isEmpty(shenfNum) && TextUtils.isEmpty(yyzzb) && TextUtils.isEmpty(yihCode) && TextUtils.isEmpty(yiji)
                && TextUtils.isEmpty(erji) && TextUtils.isEmpty(city) && TextUtils.isEmpty(quyu) && TextUtils.isEmpty(shangq) && TextUtils.isEmpty(st_time) && TextUtils.isEmpty(ed_time) && TextUtils.isEmpty(liq) && TextUtils.isEmpty(kait)) {
        ...
        bundle.putString("liq", liq);
        bundle.putString("kait", kait);
        ...
        EncryPtionHttp.getInstance(this).getHttpResult(jsonObjectObservable, new EncryPtionHttp.OnHttpResult() {
            ...
            public void onSuccessResult(JSONObject result) {
                ...
                if (code) {
                    ...
                    mDialogBotList_yiji = new DialogBotList.Builder().title("请选择").botListBeanMap(mBotListBeans_yiji).builder(WhpbShaixActivity.this);
...
//                                }
        ...
        EncryPtionHttp.getInstance(this).getHttpResult(jsonObjectObservable, new EncryPtionHttp.OnHttpResult() {
            ...
            public void onSuccessResult(JSONObject result) {
                ...
                if (code) {
                    ...
                    mDialogBotList_erji = new DialogBotList.Builder().title("请选择").botListBeanMap(mBotListBeans_erji).builder(WhpbShaixActivity.this);
        ...
        EncryPtionHttp.getInstance(this).getHttpResult(jsonObjectObservable, new EncryPtionHttp.OnHttpResult() {
            ...
            public void onSuccessResult(JSONObject result) {
                ...
                if (code) {
                    ...
                    mDialogBotList_city = new DialogBotList.Builder().title("请选择").botListBeanMap(mBotListBeans_city).builder(WhpbShaixActivity.this);
        ...
        EncryPtionHttp.getInstance(this).getHttpResult(jsonObjectObservable, new EncryPtionHttp.OnHttpResult() {
            ...
            public void onSuccessResult(JSONObject result) {
                ...
                if (code) {
                    ...
                    mDialogBotList_quyu = new DialogBotList.Builder().title("请选择").botListBeanMap(mBotListBeans_quyu).builder(WhpbShaixActivity.this);
        ...
        EncryPtionHttp.getInstance(this).getHttpResult(jsonObjectObservable, new EncryPtionHttp.OnHttpResult() {
            ...
            public void onSuccessResult(JSONObject result) {
                ...
                if (code) {
                    ...
                    mDialogBotList_shangquan = new DialogBotList.Builder().title("请选择").botListBeanMap(mBotListBeans_shagnquan).builder(WhpbShaixActivity.this);
    ...
    private void initLiqKait() {
        String whatl = mScreenLingqstate.getText().toString();
        mBotListBeans_lingqu.clear();
        for (int i = 0; i < mLingq.length; i++) {
            BotListBean botListBean = new BotListBean(mLingq[i][0], TextUtils.isEmpty(whatl) ? false : (whatl.equals(mLingq[i][0]) ? true : false), mLingq[i][1]);
            mBotListBeans_lingqu.add(botListBean);
        }
        mDialogBotList_lingqu = new DialogBotList.Builder().title("请选择").botListBeanMap(mBotListBeans_lingqu).builder(WhpbShaixActivity.this);
        mDialogBotList_lingqu.setOnTextClickListener(new DialogBotList.OnTextClickListener() {
            @Override
            public void onTextClickListenerHis(String name, String what) {
                super.onTextClickListenerHis(name, what);
                mScreenLingqstate.setText(name);
                lq_code = what;
            }
        });

        String whatll = mScreenKaitstate.getText().toString();
        mBotListBeans_kaitong.clear();
        for (int i = 0; i < mkait.length; i++) {
            BotListBean botListBean = new BotListBean(mkait[i][0], TextUtils.isEmpty(whatll) ? false : (whatll.equals(mkait[i][0]) ? true : false), mkait[i][1]);
            mBotListBeans_kaitong.add(botListBean);
        }
        mDialogBotList_kaitong = new DialogBotList.Builder().title("请选择").botListBeanMap(mBotListBeans_kaitong).builder(WhpbShaixActivity.this);
        mDialogBotList_kaitong.setOnTextClickListener(new DialogBotList.OnTextClickListener() {
            @Override
            public void onTextClickListenerHis(String name, String what) {
                super.onTextClickListenerHis(name, what);
                mScreenKaitstate.setText(name);
                kait_code = what;
            }
        });
    }
    ...
    private void showRepeatDig(final EditText mtv) {
        ...
        KeyboardUtil.hideKeyboard(mScreenLingqstate);
        mRe.requestFocus();
        if (TextUtils.isEmpty(str)) {
            ...
            mDialogBotList_repeat = new DialogBotList.Builder().title("请选择").botListBeanMap(mBotListBeans_repeat).builder(WhpbShaixActivity.this);
